...
from music.forms import EditProfileForm
...
class UserFormView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid() :
            ...
            #returns user objects if credemtials are correct
...
def edit_profile(request):
    if request.method== 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect(reverse('music:profile'))
    else:
...
def change_password(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect(reverse('music:change-password'))


    else:
        form=PasswordChangeForm(user=request.user)
        args = {'form': form}
        return render(request, 'music/change_password.html' , args)
